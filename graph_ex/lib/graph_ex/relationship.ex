defmodule GraphEx.Relationship do
  ...
  def add_relationship(connection, graph, relationship) do
    ...
    {:ok, res} = Redix.command(connection, query) # decide what to do with res
    {:ok, relationship_array } = extract_data(res)
    {:ok, List.first(relationship_array)}
    # {:ok, relationship}
  end

  def get_relationships(connection, graph, src_node \\ %GraphEx.Node{}, type \\ "", properties \\ %{}, dest_node \\ %GraphEx.Node{})  do # %Node{} -- doesn't work for some reason;
  ...
  def update_relationship(connection, graph, upd_relationship) do # can I let people change the type, src_node and dest_node? and how to enforce it otherwise (through const)?
  ...
  def delete_relationship(connection, graph, relationship_to_delete) do
